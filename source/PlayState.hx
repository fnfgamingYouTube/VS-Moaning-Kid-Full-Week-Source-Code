...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['Shitball', 0.4], //From 20% to 39%
		['joe mama', 0.5], //From 40% to 49%
		['Bruhyeet', 0.6], //From 50% to 59%
		['cringe', 0.69], //From 60% to 68%
		['Nice', 0.7], //69%
		['Good', 0.8], //From 70% to 79%
		['Great', 0.9], //From 80% to 89%
		['Sick!', 1], //From 90% to 99%
		['now thats not cringe at all!!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		if(PlayState.SONG.stage == null || PlayState.SONG.stage.length < 1) {
			switch (songName)
			{
				...
				case 'ugh' | 'guns' | 'stress':
					curStage = 'tank';
				case 'platinum' | 'everlasting' | 'retroballs':
					curStage = 'house';
				case 'chaos':
					curStage = 'house2';
		...
		switch (curStage)
		{
			...
			case 'house': //Week 8
				var bg:BGSprite = new BGSprite('house', -600, -200, 0.9, 0.9);
				add(bg);

			case 'house2': //Week 9
				var bg:BGSprite = new BGSprite('house2', -600, -200, 0.9, 0.9);
				add(bg);
